...
Phase 78 keeps this single serializer and expands forbidden authority-surface marker coverage for dispatch targets, event buses, event routers, approval queues, authorization queues, asset commands, execution callbacks, execution listeners, execution targets, and execution handlers. Validation marker strings may be split in source so the forbidden API scan can still detect real runtime surfaces without tripping on the safe marker list itself.
