# Asset Governance Certification Integration Production Review
...
Production certification requires StyLua, Selene, Rojo sourcemap, Rojo build, git diff whitespace checks, executable self-checks, and a forbidden API scan on the certification integration core.

Production evidence:

- provider name is `assetGovernanceCertificationIntegrationRuntime`
- snapshot kind is `assetGovernanceCertificationIntegrationRuntimeSnapshot`
- Bootstrap registration is after `AssetGovernanceCertificationCoordinator`
- Governance snapshot provider is `assetGovernanceCertificationIntegrationRuntime`
