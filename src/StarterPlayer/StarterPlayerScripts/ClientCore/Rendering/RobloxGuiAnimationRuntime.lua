...
local function release(recordData: any)
	...
	for _, propertyName in ipairs(recordData.properties) do
		...
		if propertyOwners[key] == recordData.animationId then
			propertyOwners[key] = nil
		end
	end
...
function Runtime.play(contract: any)
	...
	if contract.targetRevision ~= tree.revision then
		...
		return fail(
			Types.FailureType.InvalidRevision,
			{ active = tree.revision, requested = contract.targetRevision }
		)
	end
	local instance = tree.instances and tree.instances[contract.targetNodeId]
	if not instance or instance:GetAttribute("LondonEngineContractId") ~= tree.contractId then
		return fail(Types.FailureType.InvalidTarget)
	end
	local valid, reason, goals = Validator.validate(contract, instance)
	if not valid or not goals then
		counters.validationFailures += 1
		return fail(reason or Types.FailureType.InvalidContract)
	end
	if active[contract.animationId] then
		return fail(Types.FailureType.DuplicateAnimation)
	end
	...
	for propertyName in pairs(goals) do
		...
		if owner and active[owner] then
			supersededOwners[owner] = true
		end
		local ok, value = pcall(function()
			return (instance :: any)[propertyName]
		end)
		if not ok then
			return fail(Types.FailureType.InvalidGoal, propertyName)
		end
		original[propertyName] = value
	end
	table.sort(properties)
	local ownerIds = {}
	for owner in pairs(supersededOwners) do
		ownerIds[#ownerIds + 1] = owner
	end
	table.sort(ownerIds)
	for _, owner in ipairs(ownerIds) do
		if active[owner] then
			counters.superseded += 1
			cancelRecord(active[owner], "Superseded", false)
		end
	end
	...
	local tweenInfo = TweenInfo.new(
		duration,
		(Enum.EasingStyle :: any)[contract.easingStyle],
		(Enum.EasingDirection :: any)[contract.easingDirection],
		repeatCount,
		reverses,
		delayTime
	)
	local okTween, tweenOrError
	if FailureInjection.consume("TweenCreate") then
		okTween = false
		tweenOrError = Types.FailureType.TweenCreationFailed
	else
	...
	local recordData = {
		...
		original = original,
		restoreOnCancel = contract.restoreOnCancel,
		state = Types.State.Playing,
		connection = nil,
	}
	for _, propertyName in ipairs(properties) do
		propertyOwners[contract.targetNodeId .. ":" .. propertyName] = contract.animationId
	end
	active[contract.animationId] = recordData
	recordData.connection = tweenOrError.Completed:Connect(function(playbackState)
		if active[contract.animationId] ~= recordData or recordData.generation ~= generation then
			return
		end
		release(recordData)
		recordData.state = playbackState == Enum.PlaybackState.Completed and Types.State.Completed
			or Types.State.Cancelled
		if recordData.state == Types.State.Completed then
			counters.completed += 1
		else
			counters.cancelled += 1
		end
...
function Runtime.cancel(animationId: any, restoreValues: boolean?)
	...
	if type(animationId) ~= "string" or not active[animationId] then
		return fail(Types.FailureType.UnknownAnimation)
	end
...
function Runtime.cancelAll(reason: string)
	local records = {}
	for _, recordData in pairs(active) do
		records[#records + 1] = recordData
	end
	table.sort(records, function(a, b)
		return a.animationId < b.animationId
	end)
	for _, recordData in ipairs(records) do
		cancelRecord(recordData, reason, false)
	end
end
...
function Runtime.inspect()
	local ids = {}
	for animationId in pairs(active) do
	...
	return {
		...
		generation = generation,
		state = shutdown and Types.State.Shutdown
			or (#ids > 0 and Types.State.Playing or Types.State.Idle),
		motionPreference = MotionPreferences.get(),
