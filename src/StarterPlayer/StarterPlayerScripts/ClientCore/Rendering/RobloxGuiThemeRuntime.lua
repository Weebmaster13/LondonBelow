...
local counters = { registrations = 0, applications = 0, idempotent = 0, rollbacks = 0, propertiesApplied = 0, validationFailures = 0, staleRejected = 0, reconcileClears = 0 }
...
function Runtime.apply(contract: any)
	...
	for _, node in ipairs(ordered) do
		...
		for _, propertyName in ipairs(properties) do
			local okOriginal, original = pcall(function() return (node.instance :: any)[propertyName] end)
			if not okOriginal then busy = false; return fail(Types.FailureType.InvalidTarget, { nodeId = node.nodeId, property = propertyName }) end
			local okApply = pcall(function() (node.instance :: any)[propertyName] = node.styles[propertyName] end)
			if not okApply then
				local rollbackOk = true; for index = #applied, 1, -1 do local item = applied[index]; rollbackOk = pcall(function() (item.instance :: any)[item.propertyName] = item.original end) and rollbackOk end
				counters.rollbacks += 1; busy = false; return fail(rollbackOk and Types.FailureType.ApplyFailed or Types.FailureType.RollbackFailed, { nodeId = node.nodeId, property = propertyName })
			end
			applied[#applied + 1] = { instance = node.instance, propertyName = propertyName, original = original }; counters.propertiesApplied += 1
		end
	end
