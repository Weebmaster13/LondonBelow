...
function Runtime.apply(contract: any)
	...
	if not admitted then
		...
		return fail(Types.FailureType.RateLimited, admissionReason)
	end
	counters.integrityChecks += 1
	local intact, integrityReason = IntegrityGuard.verify(tree, activeContract, activeValues)
	if not intact then
	...
	local function rollbackApplied(): boolean
		...
		for index = #applied, 1, -1 do
			...
			if injectedRollback then
				counters.injectedFailures += 1
			end
			local restored = not injectedRollback
				and pcall(function()
	...
	for _, node in ipairs(ordered) do
		...
		for _, propertyName in ipairs(properties) do
			local injectedRead = FailureInjection.consume("Read")
			if injectedRead then
				counters.injectedFailures += 1
			end
			local okOriginal = false
			local original = nil
			if not injectedRead then
				okOriginal, original = pcall(function()
					return (node.instance :: any)[propertyName]
				end)
			end
			if not okOriginal then
				local rollbackOk = rollbackApplied()
				busy = false
				return fail(
					rollbackOk and Types.FailureType.InvalidTarget
						or Types.FailureType.RollbackFailed,
					{ nodeId = node.nodeId, property = propertyName }
				)
			end
			local injectedApply = FailureInjection.consume("Apply")
			if injectedApply then
				counters.injectedFailures += 1
			end
			local okApply = not injectedApply
				and pcall(function()
			...
			applied[#applied + 1] = {
				key = node.nodeId .. ":" .. propertyName,
				instance = node.instance,
				propertyName = propertyName,
				original = original,
				expected = node.styles[propertyName],
			}
	...
	for _, item in ipairs(applied) do
		activeValues[item.key] = table.freeze({
...
function Runtime.verifyIntegrity()
	counters.integrityChecks += 1
	local ok, reason = IntegrityGuard.verify(Registry.get(), activeContract, activeValues)
	if not ok then
		counters.integrityViolations += 1
...
function Runtime.inspect()
	return {
		...
		activeValueCount = (function()
			local count = 0
			for _ in pairs(activeValues) do
				count += 1
			end
			return count
		end)(),
