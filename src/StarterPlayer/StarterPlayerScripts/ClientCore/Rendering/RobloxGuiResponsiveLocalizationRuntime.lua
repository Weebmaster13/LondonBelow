...
local audit = {}
local counters =
	{ reconciliations = 0, localized = 0, missingKeys = 0, policiesApplied = 0, staleRejected = 0 }
...
function Runtime.reconcile(transaction: any, contract: any)
	...
	for _, node in ipairs(contract.nodes) do
		...
		for propertyName, descriptor in pairs(node.properties) do
			if type(descriptor) == "table" and descriptor.kind == "LocalizationReference" then
				...
				if not valid then
					return fail(Types.FailureType.InvalidPlaceholder, key)
				end
	...
	for _, plan in ipairs(plans) do
		for attribute, value in pairs(plan.attributes) do
			plan.instance:SetAttribute(attribute, value)
		end
		for propertyName, text in pairs(plan.properties) do
			local okAssign = pcall(function()
				(plan.instance :: any)[propertyName] = text
			end)
			if not okAssign then
				return fail(Types.FailureType.OwnershipViolation, propertyName)
			end
