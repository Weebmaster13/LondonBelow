...
local function validId(value: any): boolean
	return type(value) == "string"
		and #value > 0
		and #value <= Types.Limits.maxThemeIdLength
...
local function copyTokens(tokens: any): (any?, string?)
	...
	for name, value in pairs(tokens) do
		if
			type(name) ~= "string"
			or #name == 0
			or #name > Types.Limits.maxTokenNameLength
			or string.match(name, "^[a-z][a-zA-Z0-9%.%-_]*$") == nil
		then
			return nil, Types.FailureType.InvalidToken
		end
		...
		tokenCount += 1
		if tokenCount > Types.Limits.maxTokensPerTheme then
			return nil, Types.FailureType.BudgetExceeded
		end
		result[name] = value
	end
...
function Catalog.register(themeId: any, revision: any, tokens: any)
	...
	if existing and revision < existing.revision then
		return { ok = false, code = Types.FailureType.StaleRevision }
	end
	...
	if existing and revision == existing.revision then
		for name, value in pairs(copied) do
			if existing.tokens[name] ~= value then
				return { ok = false, code = Types.FailureType.RevisionConflict }
			end
		end
		for name in pairs(existing.tokens) do
	...
	if not existing then
		if count >= Types.Limits.maxThemes then
			return { ok = false, code = Types.FailureType.BudgetExceeded }
		end
		count += 1
	end
...
function Catalog.get(themeId: string)
...
function Catalog.snapshot()
	...
	for _, id in ipairs(ids) do
		entries[#entries + 1] = { themeId = id, revision = themes[id].revision }
	end
