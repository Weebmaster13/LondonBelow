...
local function validateValue(
	...
	if valueType == "string" then
		if #value > Types.Limits.MaxStringLength then
			return false, "AssetGovernanceCertificationInspection payload string exceeds limit"
		end
		if isForbiddenMarker(value) then
			return false, "AssetGovernanceCertificationInspection payload contains forbidden marker"
		end
		return true, nil
	end
	if valueType == "function" or valueType == "thread" or valueType == "userdata" then
		return false, "AssetGovernanceCertificationInspection payload contains unsafe runtime value"
	end
	if valueType ~= "table" then
		return false, "AssetGovernanceCertificationInspection payload contains unsupported value"
	end
	if seen[value] then
		return false, "AssetGovernanceCertificationInspection payload contains cycle"
	end
	if isInstanceShaped(value) then
		return false,
			"AssetGovernanceCertificationInspection payload contains instance-shaped table"
	end
	if depth > Types.Limits.MaxPayloadDepth then
		return false, "AssetGovernanceCertificationInspection payload depth exceeds limit"
	end
	count.value += 1
	if count.value > Types.Limits.MaxPayloadNodes then
		return false, "AssetGovernanceCertificationInspection payload node count exceeds limit"
	end
	seen[value] = true
	for key, nested in pairs(value) do
		...
		then
			return false, "AssetGovernanceCertificationInspection payload contains unsafe key"
		end
