...
local log = Logger.scope("ExecutionPlanningRuntime")
...
function Coordinator.initialize()
	...
	log.info("Execution Planning Runtime initialized")
...
function Coordinator.start()
	if not lifecycle.initialized then
		return result(
			false,
			"NotInitialized",
			"Execution Planning Runtime must initialize before start"
		)
	end
...
function Coordinator.plan(input: any)
...
function Coordinator.runSelfChecks()
	if lifecycle.started then
		return result(
			false,
			"AlreadyStarted",
			"Execution Planning self-checks must run before start"
		)
	end
