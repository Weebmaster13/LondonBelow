--!strict
-- Defensive serialization for Persistence Boundary schemas, diagnostics, and snapshots.
...
function Serialization.validateSerializable(
	...
	if typeof ~= nil and typeof(value) == "Instance" then
		return false, "persistence payload cannot contain Roblox Instances"
	end
	local valueType = type(value)
	if valueType == "function" or valueType == "thread" or valueType == "userdata" then
		return false, "persistence payload cannot contain unsafe runtime values"
	end
	if valueType == "string" and #value > Types.Limits.MaxPayloadStringLength then
		return false, "persistence payload string exceeds limit"
	end
	...
	if counter.count > Types.Limits.MaxPayloadNodes then
		return false, "persistence payload node count exceeds limit"
	end
	local refs = seen or {}
	if refs[value] == true then
		return false, "persistence payload cannot contain cyclic tables"
	end
	refs[value] = true
	for key, nested in pairs(value) do
