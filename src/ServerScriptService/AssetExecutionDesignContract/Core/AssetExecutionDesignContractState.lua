...
function State.registerContract(schema: any): (boolean, string?)
	...
	local referenceresponsibilities = {
	...
	for _, referenceresponsibility in ipairs(referenceresponsibilities) do
		local refsOk, refsReason = hasAll(
			referenceresponsibility[1],
			referenceresponsibility[2],
			referenceresponsibility[3]
		)
		if not refsOk then
