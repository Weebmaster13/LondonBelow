...
function Diagnostics.capture(lifecycle: any, dependencies: any)
	...
	return {
		...
		limitUsage = {
			...
			checks = limitUsage(counts.checks, Types.Limits.MaxChecks),
			blocks = limitUsage(counts.blocks, Types.Limits.MaxBlocks),
		...
		gatePosture = "schema-only gate evidence; gate records do not grant client authority or execute assets",
		...
		integrityPosture = {
			gates = "gate evidence records only",
			checks = "runtime gate check records only",
			blocks = "runtime gate block records only",
			audits = "runtime gate audit records only",
		},
		noLoadingPosture = {
			noAssetLoad = true,
			noAssetPreload = true,
