...
local function validateValue(
	...
	if valueType == "string" then
		if #value > Types.Limits.MaxStringLength then
			return false, "AssetExecutionAdapterRegistrationWorkflow payload string exceeds limit"
		end
		if isForbiddenMarker(value) then
			return false,
				"AssetExecutionAdapterRegistrationWorkflow payload contains forbidden marker"
		end
		return true, nil
	end
	if valueType == "function" or valueType == "thread" or valueType == "userdata" then
		return false,
			"AssetExecutionAdapterRegistrationWorkflow payload contains unsafe runtime value"
	end
	if valueType ~= "table" then
		return false, "AssetExecutionAdapterRegistrationWorkflow payload contains unsupported value"
	end
	if getmetatable(value) ~= nil then
		return false, "AssetExecutionAdapterRegistrationWorkflow payload contains metatable"
	end
	if seen[value] then
		return false, "AssetExecutionAdapterRegistrationWorkflow payload contains cycle"
	end
	if isInstanceShaped(value) then
		return false,
			"AssetExecutionAdapterRegistrationWorkflow payload contains instance-shaped table"
	end
	if depth > Types.Limits.MaxPayloadDepth then
		return false, "AssetExecutionAdapterRegistrationWorkflow payload depth exceeds limit"
	end
	count.value += 1
	if count.value > Types.Limits.MaxPayloadNodes then
		return false, "AssetExecutionAdapterRegistrationWorkflow payload node count exceeds limit"
	end
	seen[value] = true
	for key, nested in pairs(value) do
		if type(key) == "string" then
			if #key > Types.Limits.MaxStringLength or isForbiddenMarker(key) then
				return false,
					"AssetExecutionAdapterRegistrationWorkflow payload contains unsafe key"
			end
		elseif type(key) ~= "number" then
			return false, "AssetExecutionAdapterRegistrationWorkflow payload contains unsafe key"
		end
