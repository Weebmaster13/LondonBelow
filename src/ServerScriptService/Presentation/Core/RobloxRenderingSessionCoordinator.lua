...
local SelfChecks = require(script.Parent.RobloxRenderingSessionSelfChecks)
local Types = require(script.Parent.PresentationTypes)

local Coordinator = {}
local log = Logger.scope("RobloxRenderingSession")
...
function Coordinator.initialize()
	...
	Diagnostics.registerSampler(Types.RobloxRenderingSessionProviderName, Coordinator.inspect)
	SnapshotManager.registerProvider(
		Types.RobloxRenderingSessionProviderName,
		Coordinator.getSnapshot
	)
	initialized = true
	log.success("Roblox Rendering Session Runtime initialized")
end
...
function Coordinator.inspect()
	...
	diagnostics.coordinatorId = "RobloxRenderingSessionCoordinator"
...
function Coordinator.runSelfChecks()
	if started then
		lastSelfChecks = {
			ok = false,
			reason = "Roblox rendering session self-checks require a stopped runtime.",
		}
