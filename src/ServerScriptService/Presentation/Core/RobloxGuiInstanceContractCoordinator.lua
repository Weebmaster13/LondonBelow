...
function Coordinator.initialize()
	...
	initialized = true
	log.success("Roblox GUI Instance Contract Foundation initialized")
end
...
function Coordinator.inspect()
	local value = Runtime.inspect()
	value.coordinatorId = "RobloxGuiInstanceContractCoordinator"
	value.initialized = initialized
	value.started = started
	value.lastSelfChecks = lastSelfChecks
	return value
end
...
function Coordinator.runSelfChecks()
	if started then
		return { ok = false, reason = "self-checks require a stopped runtime" }
	end
	lastSelfChecks = SelfChecks.run()
	return lastSelfChecks
end
function Coordinator.register(contract)
	return Runtime.register(contract)
end
function Coordinator.validateContract(contractId)
	return Runtime.validateContract(contractId)
end
function Coordinator.publish(contractId)
	return Runtime.publish(contractId)
end
function Coordinator.getContract(contractId)
	return Runtime.getContract(contractId)
end
