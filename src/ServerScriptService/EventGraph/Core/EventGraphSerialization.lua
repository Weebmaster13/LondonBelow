--!strict
-- Serialization and isolation helpers for Event Graph schema records.
...
local function validate(
	...
	if isUnsafeRuntimeValue(value) then
		return false, "Event Graph payload contains unsafe runtime value"
	end
	if type(value) == "string" and #value > Types.Limits.MaxPayloadStringLength then
		return false, "Event Graph payload string exceeds limit"
	end
	if type(value) ~= "table" then
		return true, nil
	end
	if seen[value] == true then
		return false, "Event Graph payload contains cycle"
	end
	...
	nodes.count += 1
	if nodes.count > Types.Limits.MaxPayloadNodes then
		return false, "Event Graph payload node count exceeds limit"
	end
	for key, nested in pairs(value) do
