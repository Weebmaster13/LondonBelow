...
local function validateInterface(record: any): (boolean, string?)
	...
	if not isList(record.methods) then
		return false, "interface methods must be an array"
	end
	local seen = {}
	for _, method in ipairs(record.methods) do
		if not isId(method) or seen[method] then
			return false, "invalid or duplicate interface method"
		end
		seen[method] = true
	end
	return true, nil
...
function Validation.definition(definition: any): (boolean, string?)
	if type(definition) ~= "table" then
		return false, "capability definition must be a table"
	end
	for key in pairs(definition) do
		if not definitionFields[key] then
			return false, "unknown capability definition field: " .. tostring(key)
		end
	end
	for key in pairs(definitionFields) do
		if definition[key] == nil then
			return false, "missing capability definition field: " .. key
		end
	end
	if
	...
	if
		...
		or #definition.interfaces > Types.Limits.MaxInterfacesPerCapability
	then
	...
	if
		...
		or #definition.dependencies > Types.Limits.MaxDependenciesPerCapability
	then
