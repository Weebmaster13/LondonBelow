--!strict
-- Serialization and isolation helpers for Condition schema records.
...
local function validate(
	...
	if isUnsafeRuntimeValue(value) then
		return false, "Condition payload contains unsafe runtime value"
	end
	if type(value) == "string" and #value > Types.Limits.MaxPayloadStringLength then
		return false, "Condition payload string exceeds limit"
	end
	if type(value) ~= "table" then
		return true, nil
	end
	if seen[value] == true then
		return false, "Condition payload contains cycle"
	end
	...
	nodes.count += 1
	if nodes.count > Types.Limits.MaxPayloadNodes then
		return false, "Condition payload node count exceeds limit"
	end
	for key, nested in pairs(value) do
