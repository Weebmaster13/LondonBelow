--!strict
-- Diagnostics for Inventory Runtime Foundation.

local Serialization = require(script.Parent.InventorySerialization)
local Types = require(script.Parent.InventoryTypes)

local Diagnostics = {}

function Diagnostics.capture(lifecycle: any, dependencies: any)
	local state = dependencies.State.inspect()
	local validationOk, validationReason = dependencies.Validation.validate()
	local health = "Healthy"
	if not validationOk then
		health = "Unhealthy"
	elseif state.counts.validationFailures > 0 then
		health = "Warning"
	end

	return Serialization.deepCopy({
		health = health,
		validationOk = validationOk,
		validationReason = validationReason,
		initialized = lifecycle.initialized,
		started = lifecycle.started,
		lastSelfChecks = lifecycle.lastSelfChecks,
		counts = state.counts,
		limits = Types.Limits,
		mode = Types.Mode,
		noExecutionPosture = {
			noPickupExecution = true,
			noItemUseExecution = true,
			noDoorUnlocking = true,
			noPuzzleSolving = true,
			noSavePersistence = true,
			noFinalUi = true,
			noWorkspaceMutation = true,
			noRemotes = true,
			noClientAuthority = true,
			noChapterContent = true,
		},
		recentValidationFailures = state.validationFailures,
	})
end

function Diagnostics.validate(dependencies: any): (boolean, string?)
	local ok, reason = dependencies.Validation.validate()
	if not ok then
		return false, reason
	end
	local state = dependencies.State.inspect()
	if state.counts.profiles > Types.Limits.MaxProfiles then
		return false, "inventory profile count exceeds limit"
	end
	if state.counts.items > Types.Limits.MaxItems then
		return false, "inventory item count exceeds limit"
	end
	return true, nil
end

return Diagnostics
