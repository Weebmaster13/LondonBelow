...
local function forbidden(payload: any, depth: number): (boolean, string?)
	...
	if depth > Types.Limits.MaxPayloadDepth then
		return false, "AssetExecutionImplementationContract payload depth exceeds limit"
	end
	for key in pairs(payload) do
		if type(key) == "string" and FORBIDDEN_LOOKUP[string.lower(key)] == true then
			return false,
				"AssetExecutionImplementationContract payload contains forbidden field: " .. key
		end
	end
	for _, nested in pairs(payload) do
		if type(nested) == "string" and FORBIDDEN_LOOKUP[string.lower(nested)] == true then
			return false,
				"AssetExecutionImplementationContract payload contains forbidden value: " .. nested
		end
...
local function validateTags(tags: any): (boolean, string?)
	...
	for _, tag in ipairs(tags) do
		if FORBIDDEN_LOOKUP[string.lower(tag)] == true then
			return false, "tag uses forbidden AssetExecutionImplementationContract marker: " .. tag
		end
	end
	return true, nil
end
...
function Validation.contract(schema: any): (boolean, string?)
	local ok, reason =
		validateSchema(schema, "contractId", Types.SchemaType.ImplementationContract, "contract")
	if not ok then
		return false, reason
	end
	if
		not validId(schema.proposedRuntimeName)
		or not validId(schema.readinessId)
		or not validId(schema.designContractId)
...
function Validation.responsibility(schema: any): (boolean, string?)
	local ok, reason = validateSchema(
		schema,
		"responsibilityId",
		Types.SchemaType.ImplementationContractResponsibility,
		"responsibility"
	)
	if not ok then
		return false, reason
...
function Validation.boundary(schema: any): (boolean, string?)
	local ok, reason = validateSchema(
		schema,
		"boundaryId",
		Types.SchemaType.ImplementationContractBoundary,
		"boundary"
	)
	if not ok then
		return false, reason
...
function Validation.audit(schema: any): (boolean, string?)
	local ok, reason =
		validateSchema(schema, "auditId", Types.SchemaType.ImplementationContractAudit, "audit")
	if not ok then
		return false, reason
	end
	if
