...
local function validate(
	...
	if isUnsafeRuntimeValue(value) or isInstanceShaped(value) then
		return false, "AssetExecutionBoundaryReview payload contains unsafe runtime value"
	end
	if type(value) == "string" and #value > Types.Limits.MaxStringLength then
		return false, "AssetExecutionBoundaryReview payload string exceeds limit"
	end
	if type(value) ~= "table" then
		return true, nil
	end
	if seen[value] == true then
		return false, "AssetExecutionBoundaryReview payload contains cycle"
	end
	...
	nodes.count += 1
	if nodes.count > Types.Limits.MaxPayloadNodes then
		return false, "AssetExecutionBoundaryReview payload node count exceeds limit"
	end
	for key, nested in pairs(value) do
