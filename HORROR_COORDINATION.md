# Horror Coordination

Horror Orchestration emits coordination bundles. A bundle is a traceable approval-only recommendation, not an execution command.

## Bundle Examples

- Suppress scare.
- Request audio silence.
- Request lighting readability protection.
- Request environment release support.
- Request monster wait.
- Delay escalation.
- Prepare future chase pressure.

## Coordinated Systems

- Observation Engine.
- Director Ecosystem.
- Horror Director.
- Environment Director.
- Lighting Director.
- Audio Director.
- Monster Intelligence.
- Gameplay Intelligence.
- Gameplay Execution Bridge.
- Future Monster AI.
- Future Narrative Runtime.
- Future Journal/Identity Runtime.

## Authority Rule

No bundle can bypass Governance, Director approval, Observation Engine truth, or future execution validation.

## Production Rule

Bundle entries are recommendations, not commands. They must be `approvalOnly`, must set `executionAllowed = false`, and must not contain execution fields such as `execute`, `apply`, `mutate`, `workspace`, or `remote`.
